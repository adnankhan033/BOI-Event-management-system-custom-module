 ...
 *
 * @ViewsField("speaker_topic")
 */
class SpeakerTopic extends FieldPluginBase {
   ...
   */
    ...
    //kint();
        $prop = [];
        $prop['nid'] = $values->_entity->Id();
          $form = \Drupal::formBuilder()->getForm('Drupal\custom_hooks\Form\SpeakerTopicForm', $prop);
        //        $form['tasks']['#options'] = $task_options;
          $render_service = \Drupal::service('renderer');
        $form = $render_service->render($form);
       // return $form;
         return [
            '#markup' => \Drupal\Core\Render\Markup::create($form),
            '#allowed_tags' => [
              'div',
              'ul',
              'li',
            ...
            foreach($field_add_sub2 as $para_sub2)
            {
                ...
                foreach($checklists as $checklist)
                {
                    ...
                    foreach($tasks as $task)
                    {
                        ...
                        $task_options[$task->id()] = $task->field_topic->getValue()[0]['value'].$viewOutput;
                    }
